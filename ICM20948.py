...
REG_VAL_BIT_GYRO_FS_2000DPS          = 0x06  # bit[2:1] # current setting
...
REG_VAL_BIT_ACCEL_FS_16g             = 0x06  # bit[2:1] current setting
...
class ICM20948(object):
  def __init__(self,address=I2C_ADD_ICM20948):
    ...
    self._write_byte( REG_ADD_GYRO_CONFIG_1 , REG_VAL_BIT_GYRO_DLPCFG_6 | REG_VAL_BIT_GYRO_FS_2000DPS | REG_VAL_BIT_GYRO_DLPF) # changed from 1000 to 2000
    self._write_byte( REG_ADD_ACCEL_SMPLRT_DIV_2 ,  0x07)
    self._write_byte( REG_ADD_ACCEL_CONFIG , REG_VAL_BIT_ACCEL_DLPCFG_6 | REG_VAL_BIT_ACCEL_FS_16g | REG_VAL_BIT_ACCEL_DLPF) # changed FS_2g to FS_16g
    #user bank 0 register
    self._write_byte( REG_ADD_REG_BANK_SEL , REG_VAL_REG_BANK_0) 
    time.sleep(0.1)
    self.GyroOffset()
...
if __name__ == '__main__':
  import pandas as pd
  df = pd.read_csv('./outputs/motionCount.csv')
  ...
  while True:
    ...
    df.loc[len(df.index)] = [icm[0],icm[1],icm[2],icm[3],icm[4],icm[5],icm[6],icm[7],icm[8],icm[9],icm[10],icm[11]] 
    # Roll, Pitch, Yaw, AccX, AccY, AccZ, GyroX, GyroY, GyroZ, MagX, MagY, MagZ
    df.to_csv('./outputs/motionCount.csv', index=False)
